...
<div class="container-fluid px-0">
    ...
    @if(isset($planData['overview']))
        <div class="glass-card mb-4">
            <p style="color:var(--text-primary);font-size:0.9rem;line-height:1.6;">{{ $planData['overview'] }}</p>
        </div>
    @endif
                    ...
                    @foreach($day['sessions'] as $session)
                        <div class="d-flex align-items-center gap-3 p-3 mb-2 rounded-3" style="background:rgba(99,102,241,0.04);border-left:3px solid {{ $session['priority'] === 'high' ? '#ef4444' : ($session['priority'] === 'medium' ? '#f59e0b' : '#10b981') }};">
                            <div style="min-width:130px;">
                                <span style="font-size:0.75rem;font-weight:600;color:var(--card-accent);">{{ $session['time'] }}</span>
                            </div>
    ...
    @if(isset($planData['tips']))
        <div class="glass-card mb-4">
            ...
            <ul style="color:var(--text-primary);font-size:0.88rem;line-height:1.8;">
