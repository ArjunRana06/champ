...
class AiService
{
    ...
    public function chat(array $messages, string|array|null $model = null, float $temperature = 0.3, int $maxTokens = 2048, bool $useCache = true): string
    {
    ...
    public function generateJson(array $messages, string|array|null $model = null, float $temperature = 0.3, int $maxTokens = 2048, bool $useCache = true): array
    {
        $content = $this->chat($messages, $model, $temperature, $maxTokens, $useCache);
    ...
    {
        $identity = auth()->id() ?? 'anonymous';
        $serialized = json_encode($messages);

        return 'ai_chat_'.md5($identity.'|'.$serialized);
    }
    ...
    private function getFallbackResponse(array $messages): string
    {
        ...
        if (preg_match('/^(hi|hello|hey|sup|yo|namaste)\b/i', $lower)) {
            return "Hello! I'm your Study Assistant for Students. I'm experiencing some connectivity issues with the AI service right now, but I'm still here to help! Try asking your question again in a moment, or upload your study materials for personalized assistance.";
        }

        if (preg_match('/who (are|r) (you|u)/i', $lower)) {
            return "I'm your **Study Assistant for Students**! I help students learn by explaining concepts, generating quizzes, and creating study plans. The AI service is temporarily having issues, but please try your question again shortly.";
        }
